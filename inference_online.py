...
def handler_online(args, model, valid_len,  inf, dev):
    ...
    #waveform, sample_rate = torchaudio.load("/stek/corpora/LibriSpeech/test-clean/2961/960/2961-960-0000.flac")
    waveform, sample_rate = torchaudio.load("/stek/corpora/voxpopuli/transcribed_data/it/2016/20160607-0900-PLENARY-3-it_20160607-09:32:08_3.ogg")
...
def main():
    ...
    if lang == "eng":
        ...
        path = args.load_model_dir + "/english-EE-conformer"

    if lang == "it":
        args.load_model_dir = os.getcwd() + '/Italian-EE-conformer'
        path = args.load_model_dir + "/italian-EE-conformer"

    args.batch_size=1
    args.bpe_model=args.load_model_dir + "/bpe-256.model"
    args.bpe_lexicon=args.load_model_dir + "/bpe-256.lex"
    args.bpe_tokens=args.load_model_dir + "/bpe-256.tok"
    args.tokens=args.bpe_tokens
    args.lexicon=args.bpe_lexicon
    args.device="cpu"
    ...
    run(path, model=model, args=args, inf=inf)
